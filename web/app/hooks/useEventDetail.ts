...
import { getCheckin } from '@/lib/arkiv/entities/checkin'
import type {
  EventPayload,
  ArkivEntity,
  RsvpEntity,
...
export function useEventDetail(entityKey: string) {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    setError(null)
    try {
      const [ev, rs, ap, mr, ma, mc] = await Promise.all([
        getEvent(entityKey),
  ...
  return { event, rsvps, approvals, myRsvp, myApproval, myCheckin, loading, error, reload: load }
}
