...
export type MyEventStatus = 'going' | 'pending'
...
export function useMyEvents() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      // Fetch user's RSVPs and approvals in parallel
      const [rsvpResult, approvalResult] = await Promise.all([
        publicClient
          .buildQuery()
  ...
  return { myEvents, going, pending, statusMap, loading, error, reload: load }
}
